				...
				$("#demo2").paginate({
					...
					background_hover_color	: '#CFCFCF',
						...
						onChange: function(page) {
							location.href = "<%=path%>/bloglist/bloglist.jsp?pageIndex=" + page;
						}
				});
			});
</script>
...
<div id="left">
	<div class="ListItem">
    	<div class="ListItemTop">
        	<span>当前位置 博文列表</span>
            <a href="<%=path %>/edit/edit.jsp" class="edit">[写日志]</a>
        </div>
        <s:iterator value="articles">
        <ul class="ListBox">
        	<li><a href="<%=path%>/blogdetails/blogdetails.jsp?articles.articleId=<s:property value="articleId" />" target="_blank"><s:property value="articleTitle" /></a> 
        			<a href="#" class="color1"><s:property value="articlecategory.categoryName" /></a> <s:property value="articlePostTime"/></li>
        </ul>
        </s:iterator>
    </div>
    <!-- paging -->
